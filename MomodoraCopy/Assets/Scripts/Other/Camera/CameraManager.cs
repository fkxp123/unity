...
namespace MomodoraCopy
{
    public class CameraManager : Singleton<CameraManager>
    {
        ...
        {
            GameManager.instance.mainCameraObject = gameObject;
        }
        void OnDisable()
        {
            SceneManager.sceneLoaded -= OnSceneLoaded;
        ...
        void Start()
        {
            ...
            cinemachineShake = transform.GetChild(2).GetComponent<CinemachineShake>();
        }

        void LateUpdate()
        {
            if(cameraTargetBounds == null || targetCollider == null)
            {
                cameraTargetBounds = GameManager.instance.playerPhysics.transform.GetChild(1);
                targetCollider = cameraTargetBounds.GetComponent<BoxCollider2D>();
                return;
            }
            ...
            if(float.IsNaN(clampedX) || float.IsNaN(clampedY))
            {
                return;
            }
        ...
        {
            StartCoroutine(cinemachineShake.Shake(3f, .1f));
        }
